...
describe("UPDATE で quiz_sessions の CHECK 制約を破れないことの検証", () => {
	...
	it("UPDATE で completed にしつつ completed_at を NULL のままにすることを拒否", () => {
		expect(() => db.prepare("UPDATE quiz_sessions SET status = 'completed', current_question_order = NULL, answered_question_count = 5, correct_answer_count = 3, incorrect_answer_count = 2, last_answered_at = ? WHERE quiz_session_id = ?").run(NOW, "session-1")).toThrow();
	});
	...
	it("UPDATE で answered > 0 にしつつ last_answered_at を NULL のままにすることを拒否", () => {
		expect(() => db.prepare("UPDATE quiz_sessions SET answered_question_count = 1, correct_answer_count = 1, incorrect_answer_count = 0 WHERE quiz_session_id = ?").run("session-1")).toThrow();
	});

	it("UPDATE で正しい状態遷移 (in_progress → completed) は成功", () => {
		expect(() => db.prepare("UPDATE quiz_sessions SET status = 'completed', answered_question_count = 5, correct_answer_count = 3, incorrect_answer_count = 2, current_question_order = NULL, completed_at = ?, last_answered_at = ? WHERE quiz_session_id = ?").run(NOW, NOW, "session-1")).not.toThrow();
	});

	it("UPDATE で正しい状態遷移 (in_progress → abandoned) は成功", () => {
		expect(() => db.prepare("UPDATE quiz_sessions SET status = 'abandoned', current_question_order = NULL WHERE quiz_session_id = ?").run("session-1")).not.toThrow();
	});
});
...
describe("UPDATE で user_score_states の制約を破れないことの検証", () => {
	beforeEach(() => {
		insertScoreTier(db);
		db.prepare("INSERT INTO user_score_states (user_score_state_id, user_id, score_scope, idol_group_id, score_tier_id, score_total, answered_count, correct_count, updated_at) VALUES (?,?,?,?,?,?,?,?,?)").run("uss", "user-1", "overall", null, "tier-1", 10, 5, 3, NOW);
	});
