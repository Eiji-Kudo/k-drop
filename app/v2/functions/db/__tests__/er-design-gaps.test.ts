...
describe("DB 制約では守れない設計上のギャップ（アプリ側で検証必須）", () => {
	describe("quiz_answers が別クイズの選択肢を参照できてしまう", () => {
		beforeEach(async () => {
			...
			await insertQuizChoice(db, { quizChoiceId: "q2-c1", quizId: "q2", choiceOrder: 1, isCorrect: 1 });
	...
	describe("total_question_count と実際の quiz_session_questions 件数の不一致", () => {
		it("total=5 だが questions を 2 件しか作らなくても DB は許容する", async () => {
			await insertQuiz(db, { quizId: "q1" });
			await insertQuiz(db, { quizId: "q2" });
			await insertQuizSession(db, { totalQuestionCount: 5 });
			await insertSessionQuestion(db, { id: "sq1", quizId: "q1", order: 1 });
	...
	describe("score_tier の scope と user_score_states の scope の不一致", () => {
		it("overall の score_state に group scope の tier を参照させても DB は許容する", async () => {
			await insertScoreTier(db, { scoreTierId: "tier-group", tierScope: "group", tierName: "Bronze" });
			expect(() =>
				db
					.prepare(
	...
	describe("クイズに正解選択肢が 0 個でも DB は許容する", () => {
		it("全選択肢が is_correct = 0 でも挿入可能", async () => {
			await insertQuiz(db);
			for (let i = 1; i <= 4; i++) await insertQuizChoice(db, { quizChoiceId: `c${i}`, choiceOrder: i, isCorrect: 0 });
	...
	describe("quiz_session_questions の question_order に歯抜けがあっても DB は許容する", () => {
		it("order 1, 3, 5 のように非連続でも挿入可能", async () => {
			await insertQuiz(db, { quizId: "q1" });
			await insertQuiz(db, { quizId: "q2" });
			await insertQuiz(db, { quizId: "q3" });
			await insertQuizSession(db, { totalQuestionCount: 3 });
			await insertSessionQuestion(db, { id: "sq1", quizId: "q1", order: 1 });
			await insertSessionQuestion(db, { id: "sq2", quizId: "q2", order: 3 });
			await insertSessionQuestion(db, { id: "sq3", quizId: "q3", order: 5 });
	...
	describe("completed 後の不正な UPDATE を DB 制約だけでは防げないケース", () => {
		it("completed セッションの correct_answer_count を UPDATE で増やせてしまう（整合性は維持される場合）", async () => {
