...
describe("users の enum 制約", () => {
	it("不正な status を拒否", () => {
		expect(() => insertUser(db, { status: "banned" })).toThrow();
	});
	it("正当な status を受理", () => {
		insertUser(db, { userId: "a", status: "active" });
		insertUser(db, { userId: "s", status: "suspended" });
		insertUser(db, { userId: "d", status: "deleted" });
...
describe("idol_groups の制約", () => {
	...
	it("slug は一意", () => {
		insertIdolGroup(db, { idolGroupId: "g1", slug: "tw" });
		expect(() => insertIdolGroup(db, { idolGroupId: "g2", slug: "tw" })).toThrow();
	});
	it("不正な status を拒否", () => {
		expect(() =>
			db
...
describe("quiz_choices の制約", () => {
	...
	it("unique(quiz_id, choice_order)", () => {
		insertQuizChoice(db, { quizChoiceId: "c1", choiceOrder: 1 });
		expect(() => insertQuizChoice(db, { quizChoiceId: "c2", choiceOrder: 1 })).toThrow();
	});
	it("choice_order >= 1", () => {
		expect(() => insertQuizChoice(db, { choiceOrder: 0 })).toThrow();
	});
	it("is_correct は 0 or 1", () => {
		expect(() => insertQuizChoice(db, { isCorrect: 2 })).toThrow();
	});
});

describe("score_tiers の制約", () => {
	it("min_score <= max_score", () => {
		expect(() => insertScoreTier(db, { minScore: 200, maxScore: 100 })).toThrow();
	});
	it("tier_scope は overall/group のみ", () => {
		expect(() => insertScoreTier(db, { tierScope: "x" })).toThrow();
	});
	it("unique(tier_scope, tier_name)", () => {
		insertScoreTier(db, { scoreTierId: "t1" });
		expect(() => insertScoreTier(db, { scoreTierId: "t2" })).toThrow();
	});
});
...
describe("quizzes の enum 制約", () => {
	beforeEach(() => setupBaseData(db));
	it("不正な difficulty を拒否", () => {
		expect(() => insertQuiz(db, { difficulty: "extreme" })).toThrow();
	});
	it("不正な status を拒否", () => {
		expect(() => insertQuiz(db, { status: "deleted" })).toThrow();
	});
});

describe("group_categories の制約", () => {
	it("slug は一意", () => {
		insertGroupCategory(db, { groupCategoryId: "c1", slug: "kpop" });
		expect(() => insertGroupCategory(db, { groupCategoryId: "c2", slug: "kpop" })).toThrow();
	});
});
