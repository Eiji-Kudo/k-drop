...
describe("quiz_choices の境界値", () => {
	...
	it("choice_order = 1 は有効（最小値）", () => {
		expect(() => insertQuizChoice(db, { choiceOrder: 1 })).not.toThrow();
	});
	it("choice_order = -1 は拒否", () => {
		expect(() => insertQuizChoice(db, { choiceOrder: -1 })).toThrow();
	});
	it("choice_order = 999 は有効", () => {
		expect(() => insertQuizChoice(db, { choiceOrder: 999 })).not.toThrow();
	});
});
...
describe("score_tiers の境界値", () => {
	it("min_score = max_score は有効", () => {
		expect(() => insertScoreTier(db, { minScore: 50, maxScore: 50 })).not.toThrow();
	});
	it("min_score = 0, max_score = 0 は有効", () => {
		expect(() => insertScoreTier(db, { minScore: 0, maxScore: 0 })).not.toThrow();
	});
});
...
describe("quiz_session_questions の境界値", () => {
	...
	it("question_order = 0 は拒否", () => {
		expect(() => insertSessionQuestion(db, { order: 0 })).toThrow();
	});
});
