...
import { createTestDb, insertAuthIdentity, insertUser, NOW } from "./test-helper";
...
const insAuth = (id: string, userId: string, provider: string, sub: string) =>
	insertAuthIdentity(db, { authIdentityId: id, userId, provider, providerSubjectId: sub, createdAt: NOW, updatedAt: NOW });

describe("認証プロバイダの管理", () => {
	it("1 ユーザーが Google + LINE の 2 つのログイン手段を持つ", async () => {
		await insAuth("ai-g", "user-1", "google", "g-sub-1");
		await insAuth("ai-l", "user-1", "line", "l-sub-1");
		const count = (db.prepare("SELECT count(*) as c FROM auth_identities WHERE user_id=?").get("user-1") as { c: number }).c;
		expect(count).toBe(2);
	});

	it("認証手段を追加した後に 1 つ削除しても残りが機能する", async () => {
		await insAuth("ai-g", "user-1", "google", "g-sub-1");
		await insAuth("ai-l", "user-1", "line", "l-sub-1");
	...
	it("異なるユーザーが同じプロバイダで別アカウントを持てる", async () => {
		await insertUser(db, { userId: "user-2" });
		await insAuth("ai-1", "user-1", "google", "g-sub-1");
		await insAuth("ai-2", "user-2", "google", "g-sub-2");
		expect((db.prepare("SELECT count(*) as c FROM auth_identities WHERE provider='google'").get() as { c: number }).c).toBe(2);
	});

	it("ユーザー削除時に全認証手段がカスケード削除される", async () => {
		await insAuth("ai-g", "user-1", "google", "g-sub-1");
		await insAuth("ai-l", "user-1", "line", "l-sub-1");
		await insAuth("ai-a", "user-1", "apple", "a-sub-1");
