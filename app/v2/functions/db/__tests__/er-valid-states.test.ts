...
import { createTestDb, insertQuiz, insertQuizChoice, insertQuizSession, insertScoreTier, insertUser, NOW, setupBaseData } from "./test-helper";
...
describe("quizzes の正常な状態バリエーション", () => {
	...
	it.each(["easy", "normal", "hard"])("difficulty = %s を受理", async (d) => {
		await expect(insertQuiz(db, { quizId: `q-${d}`, difficulty: d })).resolves.toBeDefined();
	});
	it.each(["draft", "published", "archived"])("status = %s を受理", async (s) => {
		await expect(insertQuiz(db, { quizId: `q-${s}`, status: s })).resolves.toBeDefined();
	});
});

describe("quiz_sessions の正常な状態バリエーション", () => {
	beforeEach(async () => await setupBaseData(db));
	it("in_progress: 0問回答済み", async () => {
		await expect(insertQuizSession(db, { quizSessionId: "s1" })).resolves.toBeDefined();
	});
	it("in_progress: 途中まで回答済み", async () => {
		await expect(
	...
	it("abandoned: 0問で離脱", async () => {
		await expect(insertQuizSession(db, { quizSessionId: "s1", status: "abandoned", currentQuestionOrder: null })).resolves.toBeDefined();
	});
});
...
describe("events の正常な状態バリエーション", () => {
	...
	const ins = (id: string, o: { visibility?: string; capacity?: number | null }) =>
		db
			...
			.run(id, "user-1", "E", o.visibility ?? "public", o.capacity ?? null, "2025-03-01T00:00:00Z", "2025-04-01T00:00:00Z", NOW, NOW);
	it.each(["public", "private", "unlisted"])("visibility = %s を受理", (v) => {
		expect(() => ins(`ev-${v}`, { visibility: v })).not.toThrow();
	});
	it("capacity = NULL は許容", () => {
		expect(() => ins("ev-n", { capacity: null })).not.toThrow();
	});
	it("capacity = 1 は許容", () => {
		expect(() => ins("ev-1", { capacity: 1 })).not.toThrow();
	});
});
	...
	it.each(["joined", "waitlisted", "cancelled"])("participation_status = %s を受理", (s) => {
		expect(() =>
			db
				...
				.run(`ep-${s}`, "ev-1", "user-1", s, NOW, NOW),
		).not.toThrow();
	...
	it("overall 行を正常に挿入", () => {
		expect(() =>
			db
				...
				.run("ov", "user-1", "overall", null, "tier-1", 50, 10, 7, NOW),
		).not.toThrow();
	});
	it("group 行を正常に挿入", () => {
		expect(() =>
			db
				...
				.run("gr", "user-1", "group", "group-1", "tier-g", 30, 5, 3, NOW),
		).not.toThrow();
	});
});
...
describe("quiz_choices の正常パターン", () => {
	...
	it("4択を正常に挿入", async () => {
		for (let i = 1; i <= 4; i++) await insertQuizChoice(db, { quizChoiceId: `c${i}`, choiceOrder: i, isCorrect: i === 1 ? 1 : 0 });
