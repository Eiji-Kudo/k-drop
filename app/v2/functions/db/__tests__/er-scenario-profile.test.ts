...
import { NOW, createTestDb, insertGroupCategory, insertIdolGroup, insertUser, setupBaseData } from "./test-helper";
...
const insProfile = (userId: string, handle: string, displayName: string, bio: string | null = null) =>
	db.prepare("INSERT INTO user_profiles (user_id, handle, display_name, avatar_url, bio, fan_since, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)").run(userId, handle, displayName, null, bio, null, NOW, NOW);

describe("プロフィール管理", () => {
	beforeEach(() => insertUser(db));

	it("プロフィールをフル情報で作成", () => {
		db.prepare("INSERT INTO user_profiles (user_id, handle, display_name, avatar_url, bio, fan_since, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)").run(
			"user-1", "momo_lover", "モモ推し", "https://example.com/avatar.jpg", "TWICEのモモが大好き！", "2020-03-15", NOW, NOW,
		);
...
describe("推しグループの管理", () => {
	...
	it("複数グループを推し登録", () => {
		db.prepare("INSERT INTO user_favorite_groups (user_favorite_group_id, user_id, idol_group_id, started_supporting_on, created_at) VALUES (?,?,?,?,?)").run("f1", "user-1", "group-1", "2019-06-01", NOW);
		db.prepare("INSERT INTO user_favorite_groups (user_favorite_group_id, user_id, idol_group_id, started_supporting_on, created_at) VALUES (?,?,?,?,?)").run("f2", "user-1", "ive", "2021-12-01", NOW);
		const favs = db.prepare("SELECT idol_group_id FROM user_favorite_groups WHERE user_id=? ORDER BY created_at").all("user-1") as { idol_group_id: string }[];
	...
	it("推し登録を解除して再登録", () => {
		...
		expect(() => db.prepare("INSERT INTO user_favorite_groups (user_favorite_group_id, user_id, idol_group_id, created_at) VALUES (?,?,?,?)").run("f2", "user-1", "group-1", NOW)).not.toThrow();
	});

	it("グループ削除で推し関係も消える", () => {
		db.prepare("INSERT INTO user_favorite_groups (user_favorite_group_id, user_id, idol_group_id, created_at) VALUES (?,?,?,?)").run("f1", "user-1", "ive", NOW);
