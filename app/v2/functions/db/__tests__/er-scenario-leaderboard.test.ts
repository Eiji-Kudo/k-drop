...
const insEntry = (id: string, snapId: string, userId: string, rank: number, score: number) =>
	db
		...
		.run(id, snapId, userId, rank, score);
...
describe("スコアスナップショットからの履歴追跡", () => {
	it("日次スコアスナップショットでスコア推移を記録", () => {
		...
		).run("ss-d1", "u1", "overall", null, "2025-01-01", 100, NOW);
		db.prepare(
			"INSERT INTO user_score_snapshots (user_score_snapshot_id, user_id, score_scope, idol_group_id, snapshot_date, score_total, created_at) VALUES (?,?,?,?,?,?,?)",
		).run("ss-d2", "u1", "overall", null, "2025-01-02", 150, NOW);
		db.prepare(
			"INSERT INTO user_score_snapshots (user_score_snapshot_id, user_id, score_scope, idol_group_id, snapshot_date, score_total, created_at) VALUES (?,?,?,?,?,?,?)",
		).run("ss-d3", "u1", "overall", null, "2025-01-03", 250, NOW);
	...
	it("overall と group のスナップショットを同日に記録", () => {
		db.prepare(
			"INSERT INTO user_score_snapshots (user_score_snapshot_id, user_id, score_scope, idol_group_id, snapshot_date, score_total, created_at) VALUES (?,?,?,?,?,?,?)",
		).run("ss-ov", "u1", "overall", null, "2025-01-01", 200, NOW);
		db.prepare(
			"INSERT INTO user_score_snapshots (user_score_snapshot_id, user_id, score_scope, idol_group_id, snapshot_date, score_total, created_at) VALUES (?,?,?,?,?,?,?)",
		).run("ss-gr", "u1", "group", "group-1", "2025-01-01", 120, NOW);
