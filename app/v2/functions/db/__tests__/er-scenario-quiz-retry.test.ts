...
describe("不正解含みのクイズ完了 → スコア計算 → ドロップ報酬", () => {
	beforeEach(async () => {
		...
		await insertQuizChoice(db, { quizChoiceId: "q1-ok", quizId: "q1", choiceOrder: 1, isCorrect: 1 });
		await insertQuizChoice(db, { quizChoiceId: "q1-ng", quizId: "q1", choiceOrder: 2, isCorrect: 0 });
		await insertQuiz(db, { quizId: "q2" });
		await insertQuizChoice(db, { quizChoiceId: "q2-ok", quizId: "q2", choiceOrder: 1, isCorrect: 1 });
		await insertQuizChoice(db, { quizChoiceId: "q2-ng", quizId: "q2", choiceOrder: 2, isCorrect: 0 });
	});
...
describe("全問不正解でもセッションは正常に完了する", () => {
	it("correct=0, incorrect=2 で completed", async () => {
		await insertQuiz(db, { quizId: "q1" });
		await insertQuizChoice(db, { quizChoiceId: "c1-ng", quizId: "q1", choiceOrder: 1, isCorrect: 0 });
		await insertQuiz(db, { quizId: "q2" });
		await insertQuizChoice(db, { quizChoiceId: "c2-ng", quizId: "q2", choiceOrder: 1, isCorrect: 0 });
		insSession("s1", "in_progress", 0, 0, 0, 1, null, null);
		insQuestion("sq1", "s1", "q1", 1);
		insQuestion("sq2", "s1", "q2", 2);
