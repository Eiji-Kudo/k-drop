...
const insSession = (id: string, status: string, answered: number, correct: number, incorrect: number, cqo: number | null, completedAt: string | null, lastAnswered: string | null) =>
	db.prepare("INSERT INTO quiz_sessions (quiz_session_id, user_id, idol_group_id, status, total_question_count, answered_question_count, correct_answer_count, incorrect_answer_count, current_question_order, started_at, last_answered_at, completed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)").run(
		id, "user-1", "group-1", status, 2, answered, correct, incorrect, cqo, NOW, lastAnswered, completedAt,
	);
const insQuestion = (id: string, sessionId: string, quizId: string, order: number) =>
	db.prepare("INSERT INTO quiz_session_questions (quiz_session_question_id, quiz_session_id, quiz_id, question_order, created_at) VALUES (?,?,?,?,?)").run(id, sessionId, quizId, order, NOW);
const insAnswer = (id: string, sqId: string, choiceId: string, score: number) =>
	db.prepare("INSERT INTO quiz_answers (quiz_answer_id, quiz_session_question_id, quiz_choice_id, awarded_score, answered_at) VALUES (?,?,?,?,?)").run(id, sqId, choiceId, score, NOW);
...
describe("不正解含みのクイズ完了 → スコア計算 → ドロップ報酬", () => {
	...
	it("1問正解・1問不正解 → correct=1, incorrect=1, ドロップ報酬は正解分のみ", () => {
		...
		db.prepare("UPDATE quiz_sessions SET answered_question_count=2, correct_answer_count=1, incorrect_answer_count=1, current_question_order=NULL, status='completed', completed_at=?, last_answered_at=? WHERE quiz_session_id=?").run(NOW, NOW, "s1");

		db.prepare("INSERT INTO drop_wallets (user_id, balance, updated_at) VALUES (?,?,?)").run("user-1", 0, NOW);
		db.transaction(() => {
			db.prepare("INSERT INTO drop_transactions (drop_transaction_id, user_id, delta, reason, source_type, source_id, created_at) VALUES (?,?,?,?,?,?,?)").run("tx1", "user-1", 10, "quiz_reward", "quiz_session", "s1", NOW);
...
describe("セッション途中離脱→再挑戦→完了", () => {
	...
	it("abandoned 後に新しい in_progress セッションを作って完了できる", () => {
		...
		db.prepare("UPDATE quiz_sessions SET answered_question_count=2, correct_answer_count=2, current_question_order=NULL, status='completed', completed_at=?, last_answered_at=? WHERE quiz_session_id=?").run(NOW, NOW, "s-retry");
...
describe("全問不正解でもセッションは正常に完了する", () => {
	it("correct=0, incorrect=2 で completed", () => {
		...
		db.prepare("UPDATE quiz_sessions SET answered_question_count=2, correct_answer_count=0, incorrect_answer_count=2, current_question_order=NULL, status='completed', completed_at=?, last_answered_at=? WHERE quiz_session_id=?").run(NOW, NOW, "s1");
