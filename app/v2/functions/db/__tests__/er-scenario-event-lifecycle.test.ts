...
const insEvent = (id: string, vis = "public", cap: number | null = null) =>
	db.prepare("INSERT INTO events (event_id, created_by_user_id, title, visibility, capacity, starts_at, ends_at, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?)").run(id, "organizer", "Live", vis, cap, t(10), t(18), NOW, NOW);
const insParticipant = (id: string, eventId: string, userId: string, status = "joined") =>
	db.prepare("INSERT INTO event_participants (event_participant_id, event_id, user_id, participation_status, joined_at, updated_at) VALUES (?,?,?,?,?,?)").run(id, eventId, userId, status, NOW, NOW);
...
describe("イベント作成→グループ紐付け→参加→キャンセル→再参加", () => {
	it("ライフサイクル全体が正常に動作する", () => {
		...
		const cancelled = db.prepare("SELECT participation_status FROM event_participants WHERE event_participant_id=?").get("ep-a") as { participation_status: string };
		...
		const rejoined = db.prepare("SELECT participation_status FROM event_participants WHERE event_participant_id=?").get("ep-a") as { participation_status: string };
...
describe("非公開イベントとキャパシティ管理", () => {
	it("private イベントに capacity 付きで参加者を管理", () => {
		...
		const count = (db.prepare("SELECT count(*) as c FROM event_participants WHERE event_id=? AND participation_status='joined'").get("ev-priv") as { c: number }).c;
		expect(count).toBe(2);
	});
	...
	it("waitlisted → joined への状態遷移", () => {
		...
		const b = db.prepare("SELECT participation_status FROM event_participants WHERE event_participant_id=?").get("ep-b") as { participation_status: string };
