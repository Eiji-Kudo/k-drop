...
describe("グループカテゴリとグループの管理", () => {
	it("カテゴリ配下に複数グループを配置", async () => {
		await insertGroupCategory(db, { groupCategoryId: "cat-kpop", slug: "kpop" });
		await insertIdolGroup(db, { idolGroupId: "twice", groupCategoryId: "cat-kpop", slug: "twice" });
		await insertIdolGroup(db, { idolGroupId: "ive", groupCategoryId: "cat-kpop", slug: "ive" });
		await insertIdolGroup(db, { idolGroupId: "aespa", groupCategoryId: "cat-kpop", slug: "aespa" });
...
describe("グループのステータス管理", () => {
	...
	it("active → inactive → archived のライフサイクル", async () => {
	...
	it("archived グループのクイズも残るが、新しいクイズは追加可能", async () => {
		await insertIdolGroup(db);
		await insertQuiz(db, { quizId: "q-old" });
		db.prepare("UPDATE idol_groups SET status='archived', updated_at=? WHERE idol_group_id=?").run(NOW, "group-1");
		await expect(insertQuiz(db, { quizId: "q-new" })).resolves.toBeDefined();
...
describe("グループ削除時の影響範囲", () => {
	it("グループ削除でクイズ・推し関係・セッションが全て消える", async () => {
