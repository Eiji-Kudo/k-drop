...
import { createTestDb, insertGroupCategory, insertIdolGroup, insertQuizSession, insertScoreTier, NOW, setupBaseData } from "./test-helper";
...
describe("quiz_sessions の条件付きユニーク (in_progress のみ)", () => {
	it("同一 user/group で completed が複数存在可能", async () => {
		for (let i = 1; i <= 3; i++)
	...
	it("同一 user/group で abandoned が複数存在可能", async () => {
		for (let i = 1; i <= 3; i++) await insertQuizSession(db, { quizSessionId: `s${i}`, status: "abandoned", currentQuestionOrder: null });
		expect((db.prepare("SELECT count(*) as c FROM quiz_sessions").get() as { c: number }).c).toBe(3);
	});

	it("同一 user で別グループなら in_progress が複数可能", async () => {
		await insertGroupCategory(db, { groupCategoryId: "cat-2", slug: "jpop" });
		await insertIdolGroup(db, { idolGroupId: "group-2", groupCategoryId: "cat-2", slug: "arashi" });
		await insertQuizSession(db, { quizSessionId: "s1", idolGroupId: "group-1" });
		await expect(insertQuizSession(db, { quizSessionId: "s2", idolGroupId: "group-2" })).resolves.toBeDefined();
	});

	it("completed → 再挑戦で同一 user/group の in_progress を 1 つ作れる", async () => {
		await insertQuizSession(db, {
			quizSessionId: "s-done",
			status: "completed",
			currentQuestionOrder: null,
			answeredQuestionCount: 5,
		...
		await expect(insertQuizSession(db, { quizSessionId: "s-retry" })).resolves.toBeDefined();
	});
});
...
describe("user_score_states の条件付きユニーク (scope 別)", () => {
	...
	const ins = (id: string, scope: string, groupId: string | null, tierId: string) =>
		db
			...
			.run(id, "user-1", scope, groupId, tierId, 0, 0, 0, NOW);
	...
	it("同一 user で別グループの group 行は複数可能", async () => {
		await insertGroupCategory(db, { groupCategoryId: "cat-2", slug: "jpop" });
		await insertIdolGroup(db, { idolGroupId: "group-2", groupCategoryId: "cat-2", slug: "arashi" });
		ins("gr1", "group", "group-1", "tier-g");
...
describe("user_score_snapshots の条件付きユニーク (scope 別)", () => {
	const ins = (id: string, scope: string, groupId: string | null, date: string) =>
		db
			...
			.run(id, "user-1", scope, groupId, date, 100, NOW);
