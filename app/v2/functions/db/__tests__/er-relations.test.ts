...
describe("1:1 関係", () => {
	...
	it("users ↔ user_profiles — PK 制約で 1:1 を担保", () => {
		...
		expect(() =>
			db.prepare("INSERT INTO user_profiles (user_id, handle, display_name, created_at, updated_at) VALUES (?,?,?,?,?)").run("user-1", "h2", "U1dup", NOW, NOW),
		).toThrow();
	});
	...
	it("users ↔ drop_wallets — PK 制約で 1:1 を担保", () => {
		...
		expect(() =>
			db.prepare("INSERT INTO drop_wallets (user_id, balance, updated_at) VALUES (?,?,?)").run("user-1", 200, NOW),
		).toThrow();
	});
});
...
describe("1:N 関係の連鎖", () => {
	...
	it("quiz_sessions → quiz_session_questions → quiz_answers", () => {
		insertQuiz(db);
...
describe("多対多中間テーブル", () => {
	beforeEach(() => setupBaseData(db));
	const insEvent = () => db.prepare("INSERT INTO events (event_id, created_by_user_id, title, visibility, starts_at, ends_at, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)").run(
		"ev-1", "user-1", "Ev", "public", "2025-03-01T00:00:00Z", "2025-04-01T00:00:00Z", NOW, NOW,
	);
	...
	it("event_groups — unique(event_id, idol_group_id)", () => {
		...
		expect(() => db.prepare("INSERT INTO event_groups (event_group_id, event_id, idol_group_id, created_at) VALUES (?,?,?,?)").run("eg2", "ev-1", "group-1", NOW)).toThrow();
	});

	it("event_participants — unique(event_id, user_id)", () => {
		insEvent();
		db.prepare("INSERT INTO event_participants (event_participant_id, event_id, user_id, participation_status, joined_at, updated_at) VALUES (?,?,?,?,?,?)").run("ep1", "ev-1", "user-1", "joined", NOW, NOW);
		expect(() => db.prepare("INSERT INTO event_participants (event_participant_id, event_id, user_id, participation_status, joined_at, updated_at) VALUES (?,?,?,?,?,?)").run("ep2", "ev-1", "user-1", "waitlisted", NOW, NOW)).toThrow();
	});
});

describe("FK 参照整合性", () => {
	it("存在しない user への FK 参照を拒否", () => {
		expect(() => db.prepare("INSERT INTO auth_identities (auth_identity_id, user_id, provider, provider_subject_id, created_at, updated_at) VALUES (?,?,?,?,?,?)").run("a1", "bad", "g", "s", NOW, NOW)).toThrow();
	});

	it("group_categories の restrict delete — 子がある場合は削除拒否", () => {
		insertGroupCategory(db);
		insertIdolGroup(db);
		expect(() => db.prepare("DELETE FROM group_categories WHERE group_category_id = ?").run("cat-1")).toThrow();
	});

	it("users 削除時に auth_identities がカスケード削除", () => {
		insertUser(db);
