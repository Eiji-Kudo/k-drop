...
const addTx = (id: string, delta: number, reason: string) => {
	db.prepare("INSERT INTO drop_transactions (drop_transaction_id, user_id, delta, reason, source_type, created_at) VALUES (?,?,?,?,?,?)").run(id, "user-1", delta, reason, "system", NOW);
...
describe("複数ユーザーのウォレット独立性", () => {
	...
	it("各ユーザーの残高が独立に管理される", () => {
		db.transaction(() => {
			addTx("t1", 100, "quiz_reward");
			db.prepare("INSERT INTO drop_transactions (drop_transaction_id, user_id, delta, reason, source_type, created_at) VALUES (?,?,?,?,?,?)").run("t2", "user-2", 50, "quiz_reward", "system", NOW);
...
describe("トランザクション理由の追跡", () => {
	it("reason 別に集計可能", () => {
		...
		const quizTotal = (db.prepare("SELECT SUM(delta) as total FROM drop_transactions WHERE user_id=? AND reason='quiz_reward'").get("user-1") as { total: number }).total;
		const eventTotal = (db.prepare("SELECT SUM(delta) as total FROM drop_transactions WHERE user_id=? AND reason='event_reward'").get("user-1") as { total: number }).total;
		const consumeTotal = (db.prepare("SELECT SUM(delta) as total FROM drop_transactions WHERE user_id=? AND reason='consume'").get("user-1") as { total: number }).total;
