...
afterEach(() => { db.close(); });
const count = (table: string, where: string, params: unknown[]) => (db.prepare(`SELECT count(*) as c FROM ${table} WHERE ${where}`).get(...params) as { c: number }).c;
...
describe("users 削除時のカスケード", () => {
	beforeEach(() => {
		...
		db.prepare("INSERT INTO user_favorite_groups (user_favorite_group_id, user_id, idol_group_id, created_at) VALUES (?,?,?,?)").run("fav-1", "user-1", "group-1", NOW);
		db.prepare("INSERT INTO drop_transactions (drop_transaction_id, user_id, delta, reason, source_type, created_at) VALUES (?,?,?,?,?,?)").run("tx-1", "user-1", 10, "reward", "quiz", NOW);
	});
...
describe("events 削除時のカスケード", () => {
	beforeEach(() => {
		db.prepare("INSERT INTO events (event_id, created_by_user_id, title, visibility, starts_at, ends_at, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?)").run("ev-1", "user-1", "E", "public", "2025-03-01T00:00:00Z", "2025-04-01T00:00:00Z", NOW, NOW);
		db.prepare("INSERT INTO event_groups (event_group_id, event_id, idol_group_id, created_at) VALUES (?,?,?,?)").run("eg-1", "ev-1", "group-1", NOW);
		db.prepare("INSERT INTO event_participants (event_participant_id, event_id, user_id, participation_status, joined_at, updated_at) VALUES (?,?,?,?,?,?)").run("ep-1", "ev-1", "user-1", "joined", NOW, NOW);
	});
...
describe("leaderboard_snapshots 削除時のカスケード", () => {
	it("leaderboard_entries がカスケード削除", () => {
		...
		db.prepare("INSERT INTO leaderboard_entries (leaderboard_entry_id, leaderboard_snapshot_id, user_id, display_rank, display_score) VALUES (?,?,?,?,?)").run("le-1", "ls-1", "user-1", 1, 100);
