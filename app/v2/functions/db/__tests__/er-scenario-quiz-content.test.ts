...
describe("クイズコンテンツのライフサイクル", () => {
	it("draft → published → archived のステータス遷移", async () => {
		await insertQuiz(db, { status: "draft" });
	...
	it("published クイズに explanation を後から追加", async () => {
		await insertQuiz(db);
...
describe("選択肢の構成パターン", () => {
	...
	it("4 択クイズを正しく構成（正解 1 つ + 不正解 3 つ）", async () => {
		await insertQuizChoice(db, { quizChoiceId: "c1", choiceOrder: 1, isCorrect: 1 });
		await insertQuizChoice(db, { quizChoiceId: "c2", choiceOrder: 2, isCorrect: 0 });
		await insertQuizChoice(db, { quizChoiceId: "c3", choiceOrder: 3, isCorrect: 0 });
		await insertQuizChoice(db, { quizChoiceId: "c4", choiceOrder: 4, isCorrect: 0 });
...
describe("難易度別のクイズ管理", () => {
	it("同一グループに easy/normal/hard のクイズを作成して難易度別に取得", async () => {
		await insertQuiz(db, { quizId: "q-e", difficulty: "easy" });
		await insertQuiz(db, { quizId: "q-n", difficulty: "normal" });
		await insertQuiz(db, { quizId: "q-h", difficulty: "hard" });
	...
	it("published のクイズだけをフィルタ取得", async () => {
		await insertQuiz(db, { quizId: "q-pub", status: "published" });
		await insertQuiz(db, { quizId: "q-draft", status: "draft" });
		await insertQuiz(db, { quizId: "q-arch", status: "archived" });
