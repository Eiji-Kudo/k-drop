...
`home` `profile` `ranking` `quiz` の UI / schema / mock / formatter は `src/features` 配下へ移し、  
...
複数の子 route が同じ wrapper / guard / loader を持つ場合は `route.tsx` を追加できる（TanStack Router の機能）。  
ただし現時点の K-Drop v2 では採用しておらず、`(tabs)/quiz/` にも `route.tsx` は置いていない。tab bar の表示制御などは `__root.tsx` 側で行っている。

将来導入する場合のファイル構成例:

```text
src/routes/(tabs)/quiz/
├── route.tsx        # 共通 layout / guard / loader を集約する場合に追加
├── index.tsx
├── create.tsx
├── question.tsx
├── $sessionId.tsx
...
`home` `profile` `ranking` `quiz` のように、1つの feature を触る時の探索起点にする。
...
feature に閉じる mock は `src/features/<feature>/` 配下へ置く。ファイル数が多い場合は `mock/` サブディレクトリにまとめる。
...
- mock: ファイル数が多い feature は `src/features/<feature>/mock/` にまとめる。ファイル数が少なく命名で用途が自明な場合は feature 直下の `mock-*.ts` でもよい
- `src/features/<feature>/constants.ts`

現状の例:

- `quiz` は複数 mock ファイルを `features/quiz/mock/` にまとめている
- `ranking` は `mock-data.ts` / `mock-group-rankings-*.ts` を feature 直下に置いている
...
テストは対象ソースの隣の `__tests__/` サブディレクトリに colocate する。

例:

- `src/features/quiz/schemas/__tests__/quiz-create-schema.test.ts`
- `src/features/profile/utils/__tests__/format-fan-duration.test.ts`
- `src/lib/ux/__tests__/view-models.test.ts`

複数 route をまたぐ smoke test や app-level 結合テストだけ `src/__tests__/` に集約する。

- `src/__tests__/App.test.tsx`

## Migration Example

この方針は `home` `profile` `ranking` `quiz` に適用済みであり、新規 feature も同じ考え方で配置する。
...
- feature-local mock は `src/features/<feature>/` に置き、ファイル数が多い場合は `mock/` サブディレクトリへ集約する。app-wide mock だけ `src/mocks` に置く
