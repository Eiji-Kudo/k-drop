...
import { groupCategories, idolGroups, quizzes, quizDifficulties, appUsers, monthlyScoreHistories, events, eventGroupParticipations, eventParticipations, rankingGroups, rankingTotals, userIdolGroupScores, groupOtakuLayers, userProfiles, totalOtakuLayers, userQuizAnswers, quizChoices } from "./schema";
...
export const quizDifficultiesRelations = relations(quizDifficulties, ({many}) => ({
	quizzes: many(quizzes),
}));

export const monthlyScoreHistoriesRelations = relations(monthlyScoreHistories, ({one}) => ({
...
export const eventGroupParticipationsRelations = relations(eventGroupParticipations, ({one}) => ({
	event: one(events, {
...
export const userIdolGroupScoresRelations = relations(userIdolGroupScores, ({one}) => ({
	appUser: one(appUsers, {
...
export const groupOtakuLayersRelations = relations(groupOtakuLayers, ({many}) => ({
	userIdolGroupScores: many(userIdolGroupScores),
}));
...
export const totalOtakuLayersRelations = relations(totalOtakuLayers, ({many}) => ({
	userProfiles: many(userProfiles),
}));
